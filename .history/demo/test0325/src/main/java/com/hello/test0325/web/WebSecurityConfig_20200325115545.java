...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter /** WebSecurityConfig instence 생성 하기 위한 클래스 **/{
	...
	// @Autowired
	@Override
	protected void configure(HttpSecurity http,AuthenticationManagerBuilder auth, PasswordEncoder pe)/** HTTP 요청에 대한 웹 기반 보안 구성 */ throws Exception {
		http
			.authorizeRequests() //페이지 접근 권한 설정
				//.passwordEncoder(pe)
				.antMatchers("/", "/home").permitAll() // home 경로 권한없이 접근 가능
			...
			.formLogin() //로그인 설정, httpSession 기본적 이용
				...
				.defaultSuccessUrl("/", true) // 로그인 성공시 이동 하는 페이지 설정
